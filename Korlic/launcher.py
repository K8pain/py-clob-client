...
DEFAULT_PROGRESS_CSV = DEFAULT_REPORTS_PATH / "trade_progress.csv"
...
def _trade_stats(db_path: Path) -> dict[str, object]:
    with sqlite3.connect(db_path) as conn:
        total_trades = conn.execute("SELECT COUNT(*) FROM pseudo_trades").fetchone()[0]
        won = conn.execute("SELECT COUNT(*) FROM pseudo_trades WHERE result_class='WON'").fetchone()[0]
        lost = conn.execute("SELECT COUNT(*) FROM pseudo_trades WHERE result_class='LOST'").fetchone()[0]
        pending = conn.execute("SELECT COUNT(*) FROM pseudo_trades WHERE result_class='PENDING'").fetchone()[0]
        total_pnl = conn.execute("SELECT COALESCE(SUM(net_pnl), 0) FROM pseudo_trades").fetchone()[0]
        total_signals = conn.execute("SELECT COUNT(*) FROM events WHERE event_type='SIGNAL_DETECTED'").fetchone()[0]
...
def _append_progress_csv(path: Path, row: dict[str, object]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    write_header = not path.exists()
    with path.open("a", encoding="utf-8") as fh:
        if write_header:
            fh.write("ts_utc,total_trades,won,lost,pending,total_pnl,total_signals\n")
        fh.write(
            f"{row['ts_utc']},{row['total_trades']},{row['won']},{row['lost']},"
            f"{row['pending']},{row['total_pnl']},{row['total_signals']}\n"
        )


async def _run_all(bot: KorlicBot, logger: logging.Logger, db_path: Path, interval_seconds: float, progress_csv: Path) -> None:
    logger.info(
        "all-mode started interval_seconds=%s db_path=%s progress_csv=%s",
        interval_seconds,
        db_path,
        progress_csv,
    )
    while True:
        ...
        _append_progress_csv(progress_csv, stats)
        logger.info(
            "progress total_trades=%s won=%s lost=%s pending=%s total_pnl=%s total_signals=%s",
            stats["total_trades"],
            stats["won"],
            stats["lost"],
            stats["pending"],
...
def _print_specs() -> None:
    lines = [
        ...
        "6) Modo desatendido todo-en-uno:",
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--all", action="store_true", help="Ejecuta run-loop + progreso agregado en log/CSV.")
    parser.add_argument("--factory", help="Factory 'modulo:funcion' que retorna KorlicBot.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.all:
        if not args.factory:
            parser.error("--all requiere --factory")
    ...
    if args.command is None:
        parser.error("usa un subcomando o --all")
