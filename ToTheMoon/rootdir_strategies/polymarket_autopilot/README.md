...
`ToTheMoon.rootdir_strategies.polymarket_autopilot` es un simulador de estrategias para Polymarket que opera **solo en paper trading**.
...
- DB: `ToTheMoon/rootdir_strategies/polymarket_autopilot/data/paper_trading.db`
- Log: `ToTheMoon/rootdir_strategies/polymarket_autopilot/logs/polymarket-autopilot.log`

## Ejecución rápida (ahora con salida visible)

```bash
python -m ToTheMoon.rootdir_strategies.polymarket_autopilot.runner
```
...
[polymarket-autopilot] resumen guardado en: ToTheMoon/rootdir_strategies/polymarket_autopilot/logs/polymarket-autopilot.log
```
...
```bash
python -m ToTheMoon.rootdir_strategies.polymarket_autopilot.runner --mode once
```

### 2) Scheduler diario (08:00)

```bash
python -m ToTheMoon.rootdir_strategies.polymarket_autopilot.runner --mode scheduler
```

### 3) Cambiar carpeta base (data/logs)

```bash
python -m ToTheMoon.rootdir_strategies.polymarket_autopilot.runner --base-path /tmp/polymarket-autopilot
```
