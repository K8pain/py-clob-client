...
def test_public_clob_get_orderbook_does_not_call_server_time(monkeypatch):
    from Korlic_v2.factory import PublicClobClient
...
def test_signal_engine_prunes_and_caps_dedupe():
    ...
    engine.dedupe = {"m1:yes:1", "m2:yes:2", "m3:yes:3"}

    engine.prune_to_active_markets({"m1", "m3"})

    assert "m2:yes:2" not in engine.dedupe
