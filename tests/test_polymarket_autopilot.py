...
def test_generate_tail_bonding_and_spread_signals(tmp_path: Path) -> None:
    ...
    autopilot = PolymarketAutopilot(store=store, log_directory=tmp_path, config=StrategyConfig())
...
def test_publish_window_summary_writes_channel_report(tmp_path: Path) -> None:
    class FakeResponse:
    ...
    summary_path = autopilot.publish_window_summary(as_of=date(2026, 3, 24), lookback_days=180)
...
def test_runner_run_once_prints_progress(tmp_path: Path, capsys) -> None:
    from ToTheMoon.strategies.polymarket_autopilot import runner

    class StubAutopilot:
        def run_simulation(self, cycles: int) -> dict[str, int]:
            assert cycles == 90
            return {"cycles": 90, "snapshots": 300, "executed_trades": 10, "closed_positions": 2}

        def publish_window_summary(self, lookback_days: int) -> Path:
            assert lookback_days == 90
            return tmp_path / "polymarket-autopilot.log"

    summary_path = runner.run_once(StubAutopilot(), simulation_days=90)
    captured = capsys.readouterr().out

    assert summary_path == tmp_path / "polymarket-autopilot.log"
    assert "simulación completada" in captured
